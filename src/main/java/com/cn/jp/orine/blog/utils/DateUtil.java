...
public class DateUtil {
	...
	public static Map getApartTime(String begin, String end) {
		...
		if (temp.length > 1 && temp2.length > 1) {
			...
			begins.set(StringUtil.toInt(temp[0]), StringUtil.toInt(temp[1]), StringUtil.toInt(temp[2]));
	...
	public static void main(String[] a) {
		System.out.println(rollHour(new Date(), 1));
	}
	...
	public static String rollDay(Date addTime, String timeLimitDay) {
		Date t = DateUtil.rollDate(addTime, 0, 0, StringUtil.toInt(timeLimitDay));
		return t.getTime() / 1000 + "";
	}
	 ...
	 * @return
	 */
	public static Date currWeekFirstDay(Date d) {
	 ...
	 * @return
	 */
	public static Date currDay(Date d) {
		Calendar c = new GregorianCalendar();
		c.setFirstDayOfWeek(Calendar.MONDAY);
		c.setTime(d);
		c.set(c.get(Calendar.YEAR), c.get(Calendar.MONTH), c.get(Calendar.DATE), 0, 0, 0);
